...
import { INSTALL_VERSION } from '../../lib/version.ts';

export const gradleCode = `repositories {
    mavenCentral()
}

dependencies {
    implementation("io.github.bbobbogi:stream4j:${INSTALL_VERSION}")
}`;
...
export const quickStartCode = `StreamChat chat = new StreamChatBuilder()
...
[Maven Central의 정식 release](https://central.sonatype.com/artifact/io.github.bbobbogi/stream4j)
에서 자동으로 가져오며, 페이지 로드 후에는 GitHub Releases 정보로도 갱신됩니다 (현재: <code data-stream4j-install-version>{INSTALL_VERSION}</code>).

### Gradle (Kotlin DSL)

<Code code={gradleCode} lang="kotlin" />

### Maven

<Code code={mavenCode} lang="xml" />
...
<Code code={quickStartCode} lang="java" />
